...
$(document).ready(function() {
	//alert('debug1');
	hide_select_btns();
	$.get('http://127.0.0.1:8000/users/is_superuser', function(data) {//is_super_user
	...
	$.get('http://127.0.0.1:8000/users/folder_list',function(rp) {
		//get folders
		...
		if (rp.data.folderList.length==0){	//debuging
			...
			//$.get('http://127.0.0.1:8000/folders/info/:'+data.data);
			//end get
			var count=0;
			$.each(rp.data.folderList,function(key,val){
				$.get('http://127.0.0.1:8000/folders/info/'+val._id,function(res){
